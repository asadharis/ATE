...
get.est.simple <- function(ini1, ini2, X, Y, Ti, theta, max.iter = 100, tol = 0.001, bt.a = 0.3, bt.b = 0.5,
  verbose = TRUE) {
  ...
  # Implement the BFGS algorithm to get the optimal lambda_p and correspoding weights \hat{p}_k
  p.hat <- cpp_quasi_newt(ini1, umat, ubar, Ti, theta, bt.a, bt.b, max.iter, tol)

  if (verbose) {
    cat("\nRunning BFGS algorithm for estimating Weights q: \n")
  }

  # Implement the BFGS algorithm to get the optimal lambda_q and correspoding weights \hat{q}_k
  q.hat <- cpp_quasi_newt(ini2, umat, ubar, 1 - Ti, theta, bt.a, bt.b, max.iter, tol)
  ...
  res.l <- list(lam.p = lam.p, lam.q = lam.q, weights.p = w.p, weights.q = w.q, Y1 = Y_one, Y0 = Y_zero, tau = tau.hat,
    conv = conv)
  return(res.l)
}

# Main function to obtain the point estimate for ATT
get.est.ATT <- function(ini2, X, Y, Ti, theta, max.iter = 100, tol = 0.001, bt.a = 0.3, bt.b = 0.5, verbose = TRUE) {
  ...
  # Return list
  res.l <- list(lam.q = lam.q, weights.q = w.q, Y1 = Y_one, Y0 = Y_zero, tau = tau.hat, conv = conv)
  return(res.l)
}

# Main function to obtain the point estimate for multiple treatment effect.
# Much of the syntax is the same as
# for the binary case
get.est.MT <- function(ini.mat, X, Y, Ti, theta, max.iter = 100, tol = 0.001, bt.a = 0.3, bt.b = 0.5, verbose = TRUE) {
  ...
  for (j in 0:(J - 1)) {
    ...
    pj.hat <- cpp_quasi_newt(ini.mat[j + 1, ], umat, ubar, temp.Ti, theta, bt.a, bt.b, max.iter, tol)
  ...
  # Rerutn result.
  res.l <- list(lam.mat = lam.mat, weights.mat = weights.mat, Yj.hat = Yj.mat, conv = conv)
  return(res.l)
}
