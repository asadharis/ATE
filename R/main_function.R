# This is the main function available to the user This creates the object with some routine checks This also
# gives us the point estimates and can be used with some summary and plot functions
ATE <- function(Y, Ti, X, theta = 1, ATT = FALSE, verbose = FALSE, max.iter = 100, tol = 1e-10, initial.values = NULL, 
  ...
  J <- length(unique(Ti))
  # The totoal length of the parameter vector Recall: Using the notation of the Package Vignette we have u_K(X)
  # = cbind(1,X)
  K <- ncol(X) + 1
  
  # In some cases if we have a data.frame we need to convert it into a numeric matrix
  ...
  # If the inital estimates is not provided we generate simple initial parameters
  ...
  if (gp == "simple") {
    ...
    est <- get.est.simple(ini1, ini2, X, Y, Ti, theta, max.iter, tol, bt.a, bt.b, verbose)
    
  } else if (gp == "ATT") {
    ini2 <- initial.values
    est <- get.est.ATT(ini2, X, Y, Ti, theta, max.iter, tol, bt.a, bt.b, verbose)
  } else if (gp == "MT") {
    est <- get.est.MT(initial.values, X, Y, Ti, theta, max.iter, tol, bt.a, bt.b, verbose)
  }
  ...
  # Estimate the variance covariance matrix of E[Y(1)] and E[Y(0)]
...
# S3 print method for class ATE The function prints the point estimates and
...
# S3 summary method for ATE THis function calculates the SE, Z-statistic and confidence intervals and P
# values
summary.ATE <- function(object, ...) {
  
  # For binary treatment we calculate the variance of EY(1) - EY(0). Using the formula var(a-b) =
  ...
  if (object$gp == "simple" || object$gp == "ATT") {
    ...
    # The case of multiple treatments Now we do not have a specific variable like EY(1)-EY(0) So we just obtain
  ...
  # Create a coefficient matrix which we can print later to make it look like the out put of summary for lm/glm
  # etc.
  coef <- cbind(Estimate = object$est, StdErr = se, `95%.Lower` = Ci.l, `95%.Upper` = Ci.u, Z.value = z.stat, 
    p.value = p.values)
  
  if (object$gp == "simple") {
    res <- list(call = object$call, Estimate = coef, vcov = object$vcov, Conv = object$conv, Weights.p = object$weights.p, 
      weights.q = object$weights.q)
    
  } else if (object$gp == "ATT") {
    res <- list(call = object$call, Estimate = coef, vcov = object$cov, Conv = object$conv, weights.q = object$weights.q)
    
  } else {
    res <- list(call = object$call, Estimate = coef, vcov = object$cov, Conv = object$conv, weights = object$weights.mat)
  }
...
# A Print method for 'summary.ATE' uses the coefficient matrix to give us print statements like the summary
# function of lm/glm.
...
# A simple function to evaluate the emiprical CDF This function allows us to calculate a weighted emiprical
# CDF too.
...
# The S3 plot function for ATE This function plots the empirical CDF for all the covariates for the different
# tretment groups And the weighted eCDF after covariate balancing
plot.ATE <- function(x, ...) {
  object <- x
  Ti <- object$Ti
  ...
  if (object$gp == "simple" || object$gp == "ATT") {
    ...
    for (i in 1:ncol(x1)) {
      # Plot the first covariate Allow user to sequentially view subsequent plots
      if (i == 2) 
        par(ask = TRUE)
      ...
      if (length(unique(object$X[, i])) == 2) {
        Treatment <- x1[, i]
        Placebo <- x0[, i]
        # First plot the unweighted case This function plots the means for each treatment and covairate arm
        par(mfrow = c(1, 2))
        plot(c(0.5, 1, 2, 2.5), c(2, mean(Treatment), mean(Placebo), 2), pch = 16, cex = 1.5, ylim = c(0, 
          1), ylab = "Mean of group", xlab = "", col = c("blue", "red"), main = "Unweighted", xaxt = "n")
        ...
        # Now we plot the weighed means for treatment and placebo group
        if (!ATT) {
          new_treat <- sum(w.p * Treatment)
          new_control <- sum(w.q * Placebo)
        } else {
          new_treat <- mean(Treatment)
          new_control <- sum(w.q * Placebo)
        }
        plot(c(0.5, 1, 2, 2.5), c(2, new_treat, new_control, 2), pch = 16, cex = 1.5, ylim = c(0, 1), 
          ylab = "Mean of group", xlab = "", col = c("blue", "red"), main = "Weighted", xaxt = "n")
        axis(side = 1, at = c(1, 2), labels = c("Treatment", "Control"))
        if (ATT) {
          abline(h = mean(Treatment), lty = 2)
        } else {
        ...
        # Obtain the range and initialize a sequene at which we will plot the CDF
        ...
        par(mfrow = c(1, 2))
        plot(my.seq, temp1, cex = 0.4, pch = 16, type = "l", lty = 1, col = "red", xlab = names[i], ylab = "empirical CDF", 
        ...
        plot(my.seq, temp1, cex = 0.4, pch = 16, type = "l", lty = 1, col = "red", xlab = names[i], ylab = "emperical CDF", 
    ...
    ################## Case 2: Multiple treatment Arms###################
  } else {
    ...
    p <- ncol(object$X)
    # We generalize the case of binary treatment to J treatment arms Most of the syntax is the same here
    for (i in 1:p) {
      if (i == 2) 
        par(ask = TRUE)
      
      # Again, for binary covariates
      if (length(unique(object$X[, i])) == 2) {
        J <- object$J
        par(mfrow = c(1, 2))
        plot(c(0:(J - 1)), c(mean(object$X[Ti == 0, i]), rep(2, J - 1)), pch = 16, cex = 1.5, ylim = c(0, 
          1), ylab = "Mean of group", xlab = "Treatment group", col = 1, main = "Unweighted", xaxt = "n", 
          xlim = c(-0.5, J - 1 + 0.5))
        ...
        plot(c(0:(J - 1)), c(sum(object$X[Ti == 0, i] * wgt[1, Ti == 0]), rep(2, J - 1)), pch = 16, cex = 1.5, 
          ylim = c(0, 1), ylab = "Mean of group", xlab = "Treatment group", col = 1, main = "Weighted", 
        ...
        par(mfrow = c(1, 2))
        plot(my.seq, temp0, cex = 0.4, pch = 16, type = "l", lty = 1, col = 1, xlab = names[i], ylab = "emperical CDF", 
          main = "Unweighted emperical CDF")
        ...
        plot(my.seq, temp0, cex = 0.4, pch = 16, type = "l", lty = 1, col = 1, xlab = names[i], ylab = "emperical CDF", 
          main = "Weighted emperical CDF")
        for (j in 1:(object$J - 1)) {
          temp <- sapply(my.seq, my.ecdf, x = object$X[object$Ti == j, i], weights = wgt[j + 1, Ti == 
          j])
          lines(my.seq, temp, cex = 0.4, pch = 16, lty = j + 1, col = j + 1)
        }
      }
