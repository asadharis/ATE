###################NOTE#############################
#As for point estimates, we consider the three different
#cases oneat a time.
###################NOTE##############################

#We largely use the Notation of the Package Vignette
...
#This function obtains the big covariance matrix for the parameter theta
#of Section 2.4
get.cov.simple<- function(u, Y, Ti, obj,theta){
  ...
  #Again, we follow the notation of the package Vignette
  ...
  #Calculate the bread of the sandwich estimator using
...
#The EE equations for estimate the average treatment effect on
#the treated. ATT
...
get.gk.MT<- function(u,Y,Ti,lams, taus,theta, ulams ){
  ...
  #For each treatment arm we obtain the EE for lamba_j and
  #tau_j  = E[Y(j)]
  for(j in 1:J){
    ulam<- ulams[,j]
...
estimate_variance<- function(object){
  ...
  if(object$gp == "simple"){
    ...
    #Again the lower right corner is for our relevant
    #parameters
